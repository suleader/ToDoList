...
function addNewTask(){
    ...
    var dateInput = document.querySelector('.task-date-input'); // 獲取日期輸入
    var taskValue = taskInput.value.trim();
    var dateValue = dateInput.value; // 獲取日期輸入的值

    if(!taskValue.length){ return; } // 確認任務不是空的

    var obj = { 
        task: taskValue,
        dueDate: dateValue, // 保存日期
        tag: tag,
        done: false
    };
    ...
    dateInput.value = ''; // 清空日期輸入
    printList();
}

// 過濾任務
function filterTasks() {
    var selectedTag = document.getElementById('filter-by-tag').value;
    var filteredTasks = taskList.filter(function(task) {
        return selectedTag === 'all' || task.tag === selectedTag;
    });
...
function sortTasks(tasks) {
    var sortOrder = document.getElementById('sort-by').value;
...
function updateDisplay() {
    ...
    printList(sortedTasks); // 修改 printList 以接受任務陣列作為參數
}
...
function printList(tasks){
    ...
    for(var i = 0; i < taskList.length; i++){
        if(taskList[i].tag == select || select == 'all'){  // 篩出和選擇的tag相符的對象
            ...
            // 增加一個顯示截止日期的元素
            var dueDateDisplay = document.createElement('span');

            tagDisplay.textContent = taskList[i].tag;
            dueDateDisplay.textContent = taskList[i].dueDate;
            ...
            check.addEventListener('click', function() {
                // 切换任務的完成狀態
                var taskIndex = parseInt(this.parentElement.parentElement.dataset.num);
            ...
            taskArea.appendChild(dueDateDisplay);
...
// 添加事件监听器
document.getElementById('filter-by-tag').addEventListener('change', updateDisplay);
